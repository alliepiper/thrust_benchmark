...
template <typename T>
static void BM_exclusive_scan_reuse_tmp(benchmark::State &state)
{
  using byte = cuda::std::uint8_t;
...
BENCHMARK_TEMPLATE(BM_exclusive_scan_reuse_tmp, int)
  ->UseManualTime()
  ->Range(1 << 12, 1ll << 28);
BENCHMARK_TEMPLATE(BM_exclusive_scan_reuse_tmp, float)
  ->UseManualTime()
  ->Range(1 << 12, 1ll << 28);
